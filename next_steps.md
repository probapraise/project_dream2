...
- 현재 활성 인물 체계:
  ...
  - 코어 캐스트 = NC-0001만 등록 (`population/core_cast/NC-0001_P-1027.md`)
  - NC-0002~ 조연: 미설계 (다음 작업)
- 인덱스 상태 스냅샷:
  - `uninstantiated=3599`, `named=1`
  - named: `P-1027` (NC-0001, 키리온 렌바렌)
- P-* 슬롯 현재 필드:
  - `id / background_type / dorm / grade / mana_color / tower / major / vocation`
  - 서명귀족만: `noble_house`
  - 비인간만: `race`
  - 외국인만: `origin`
  - `b5 / derived`
...
### 다음 세션 첫 3단계 (권장)
1. **커뮤니티 레이어 B 방법론 구현 시작** — 실제 커뮤니티 글 1건 가져와서 ATOM-001 추출 및 저장 형식 확정
2. **NC-0002~ 조연 코어 캐스트 설계** — 주인공 관계망 기준 (적대/지지/멘토 포지션)
3. **인덱스 재생성** — `python3 scripts/rebuild_character_index_v2.py` (새 필드 grade/tower/major 등 반영)
...
- 출력: `worldbible_refined_bundle_20260303/population/` (P-0001~P-3600)
...
- 준비 산출물: `world_ops/batch/` 일체

### Step 5. character_index 좌표 레지스트리 초기화
- 상태: 완료 (2026-03-04)
- 기존 CH-* 162개 폐기 / `character_index_v2.md` 갱신

### Step 6. 핵심 인물 선제 조형 (구 버전 → 폐기 후 재설계)
- 상태: 완료 후 전면 재설계 (2026-03-04 세션 2)
- 구 NC-0001~NC-0007 (P-0867 등 7명): 전면 삭제, core_cast_bootstrap_v1 reset
- **현재 상태: NC-0001(주인공)만 신규 등록, 나머지 미설계**

### Step 7. P-* 학생 슬롯 추가 필드 배정
- 상태: 완료 (2026-03-04 세션 2) — 버그 수정 포함
- 실행: `python3 scripts/add_student_fields.py` (SEED=42)
- **버그 수정**: 초기 버전은 grade/tower/vocation이 동일 셔플 사용 → 완전 상관 문제
  - 수정: 각 필드 배정 직전 독립 `rng.shuffle(idx)` 호출
- 추가 필드: grade / mana_color / tower / major / vocation / noble_house / race / origin
- 배분: 모든 정원 정확히 충족, 1학년~보라탑 등 모든 조합 자연 분포 확인

### Step 7-B. foreigner origin 배정
- 상태: 완료 (2026-03-04 세션 2)
- 실행: `python3 scripts/assign_foreigner_origin.py` (SEED=42)
...
- 산출물: `population/core_cast/NC-0001_P-1027.md`
...
분해: 3레이어 추출
  ...
  ↓
저장: ATOM-NNN (YAML 형식, community_grammar_layer_b.md)
  ↓
...
#### 미결 질문
- 레이어 B가 각인광장 **전체**에 깔리는지, **특정 공간/게시판**에만 국소적으로 존재하는지 → 결정 필요
  - 이 결정에 따라 시뮬레이션 반영 범위가 달라짐

#### 저장 위치 (예정)
- `worldbible_refined_bundle_20260303/docs/community_grammar_layer_b.md` (미생성)
...
- 레이어 A/B 각인광장 공간 분리 여부 결정 (Step 8 선행)
...
| `docs/core_cast_bootstrap_v1.md` | NC-* 코어 캐스트 현황 |
| `scripts/add_student_fields.py` | P-* 추가 필드 배정 (grade/tower/major 등) |
| `scripts/assign_foreigner_origin.py` | foreigner origin 배정 |
| `scripts/generate_nonstudent_slots.py` | NS-* 비학생 655명 생성 |
