...
example:
  bash scripts/ops/world_ops_compile_execution_views.sh RUN-20260304-SIM world_ops/templates/execution_call_spec.env
EOF
}
...
BUNDLE_DIR="${3:-${ROOT_DIR}/world/live}"
...

{
  ...
  echo "- allowed_paths_file: artifacts/runs/${RUN_ID}/manifests/${call_id}.allowed_paths.txt"
  echo "- blocked_paths_file: artifacts/runs/${RUN_ID}/manifests/${call_id}.blocked_paths.txt"
  echo
