...
Dorm field policy:
- Initial bootstrap sets `dorm=UNASSIGNED` for all slots.
- Canonical dorm assignment must be done by `scripts/population/recompute_dorms.py`
  after role-related fields are populated.
        ...
        rows.append(
            {
                ...
                "crest_vision_holder": background_seq[i - 1] == "signature_noble",
                "dorm": INITIAL_DORM,
                "b5": b5,
        ...
        for row in rows:
            ...
            writer.writerow(
                [
                    row["id"],
                    row["background_type"],
                    "true" if bool(row["crest_vision_holder"]) else "false",
                    row["dorm"],
                    f"{b5['O']:.2f}",
                    f"{b5['C']:.2f}",
                    f"{b5['E']:.2f}",
    ...
    for row in rows:
        ...
        content = "\n".join(
            [
                ...
                f"crest_vision_holder: {'true' if bool(row['crest_vision_holder']) else 'false'}",
                f"dorm: {row['dorm']}",
                "b5:",
...
def print_stats(rows: list[dict[str, object]]) -> None:
    ...
    for key, value in sorted(dorm_counts.items()):
        print(f"  {key}: {value}")
...
def parse_args() -> argparse.Namespace:
    ...
    default_out_dir = repo_root / "world/live" / "population"
