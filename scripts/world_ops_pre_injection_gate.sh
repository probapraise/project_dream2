...
example:
  bash scripts/world_ops_pre_injection_gate.sh RUN-20260304-SIM world_ops/templates/execution_call_spec.env /tmp/payload.txt
EOF
}
...
BUNDLE_DIR="${4:-${ROOT_DIR}/worldbible_refined_bundle_20260303}"
...
if [[ ! -f "$VIEW_FILE" ]]; then
  notes+=("compiled view 없음: runs/${RUN_ID}/views/${call_id}.view.md")
  fail_count=$((fail_count + 1))
fi

if [[ ! -f "$ALLOWED_FILE" ]]; then
  notes+=("allowed paths manifest 없음: runs/${RUN_ID}/manifests/${call_id}.allowed_paths.txt")
  fail_count=$((fail_count + 1))
fi

expected_rel="runs/${RUN_ID}/views/${call_id}.view.md"
payload_references_view="no"
if grep -Fq "$expected_rel" "$PAYLOAD_FILE" || grep -Fq "$VIEW_FILE" "$PAYLOAD_FILE"; then
...
RAW_SOURCE_REGEX='(^|[^[:alnum:]_])(world_bible|characters|docs|layer_b|quarantine)/|worldbible_refined_bundle_20260303/(world_bible|characters|docs|layer_b|quarantine)/'
...

{
  ...
  echo "## outputs"
  echo "- gated_payload: runs/${RUN_ID}/payloads/${call_id}.gated.txt"
  echo "- report: runs/${RUN_ID}/gates/${call_id}_pre_injection_gate.md"
} > "$GATE_REPORT"
