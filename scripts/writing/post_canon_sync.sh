...
echo "5. required live docs update: narrative_state, episode_deltas, style_bible"
echo "6. conditional live docs update: story_arcs, foreshadow_registry, pre_academy_checkpoint_plan"
echo "7. manual live docs check: master_map"
...
if [[ "$episode_id" =~ ^ep([0-9]+)$ ]]; then
  ...
  if [[ -d "$repo_root/artifacts/writing/episodes/$next_episode_id" ]]; then
    echo "8. next-episode packet audit: python3 scripts/writing/audit_prompt_packet.py $next_episode_id"
    echo "9. next-episode semantic audit: python3 scripts/writing/audit_semantic_continuity.py $next_episode_id"
  fi
fi
