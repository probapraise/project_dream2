...
mapfile -t deleted_files < <(git -C "$ROOT_DIR" diff --cached --name-status | awk '$1=="D" {print $2}' | grep '^worldbible_refined_bundle_20260303/' || true)
...
gate_file="${ROOT_DIR}/world_ops/sessions/${CHANGE_ID}_deletion_gate.md"
if [[ ! -f "$gate_file" ]]; then
