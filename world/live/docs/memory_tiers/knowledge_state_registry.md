...
- must_not_dump_to_reader_yet:
  ...
  - 조기 수련 적발 프로토콜 전부
- latest shift:
  - `ep004`에서 계승조회식이 `2년 뒤 일정`이 아니라 바로 다음 공식 판정선으로 끌어올려졌고, 키리온도 왕실 소집과 혈통 판정의 무게를 더 분명히 이해하게 됐다.
- next volatility:
  - `ep005`에서는 이 규칙이 추상 설명이 아니라 현장 절차와 가족 반응으로 체감될 것이다.
...
- next volatility:
  - 이번 소집은 키리온 혼자라, 리리아가 없는 공간에서 `자연스러움`을 어떻게 유지할지가 다시 별개 문제가 된다.
...
- latest_shift:
  - `ep004`에서 왕실 소집, 문장비전 판정, 모계 그라비온 혈통 때문에 키리온의 성이 바뀔 가능성까지 서사 전면에 올라왔다.
- next_volatility:
  - `ep005`는 이 지식이 추상 설명이 아니라 키리온의 실시간 불안과 가족 반응으로 검증되는 구간이다.
