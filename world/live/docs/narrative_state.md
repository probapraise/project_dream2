...
- macro_position: Part 1 오프닝 / 렌바렌 저택 생존 부트스트랩 / 하급 아카데미 진입 전
- default_load_order:
...
- 현재 구간의 기능:
  - 하급 아카데미 진입 전, 가문/가정/행정 구조를 독자에게 익히게 하는 준비 구간
  - 코어 캐스트 대량 투입 전, 주인공 1인 시점의 생존 규칙과 장기 공론장 욕망의 씨앗을 먼저 고정하는 구간
- 현재 배치 목표:
  - 아르케이온 권역 첫 진입(12세 하급 아카데미 입학식/첫 수업/환경 노출)은 잠정적으로 `ep010` 전후를 목표로 둔다.
  - 본격적인 학술원 본과 서사는 `15세 본과 진입성취평가 -> 입학 즉시 전공 선택` 시점에서 연다.
...
- 하급 아카데미 입학은 멀리 있는 목표로만 남기고, 당장은 저택 내부 생존과 관찰 회피를 우선한다.
- 다만 장기적으로는 `12세 하급 아카데미에서 게시판 설계의 병목 파악 -> 3년 스킵 -> 15세 본과 진입성취평가와 입학 즉시 각인술 선택` 구조를 염두에 둔다.
