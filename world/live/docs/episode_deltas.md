...
- state_changes:
  ...
  - 드물게 데리온이 자리를 비우는 날이 있다는 사실이 새 빈틈으로 심어졌고, 그날마다 리리아와 집사실 앞이 다음 정보/게시판 욕망의 재점화 지점으로 떠오른다.
- activated_refs:
  - 없음
- closed_refs:
  - 없음
- carry_forward:
  - `ep004`는 이미 시작된 루틴의 드문 빈틈, 즉 `데리온 부재일`을 발판으로 리리아와 함께 게시판 고도화/댓글 욕망을 다시 점화하는 화가 되어야 한다.
