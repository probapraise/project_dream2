...
- style_constitution(S-01~S-10)은 **전역** 문체 규칙이다. VFP는 **개별 인물** 규칙이다
...
voice_fingerprint:
  version: 1
  extracted_from:
    - "출처1 (예: ep000_prologue/revision_v1)"
    - "출처2 (예: simrun-003)"
...
| style_constitution (S-01~S-10) | **전역** 문체 규칙. VFP는 **개별** 인물 규칙. 충돌 시 style_constitution 우선 |
...
```
1. style_constitution (최상위 — 전역 문체)
