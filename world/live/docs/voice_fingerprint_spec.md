...
- `house_rules.md`는 **전역** 문체 규칙이다. `style_selection_vN.md`는 **회차별 선택 패턴**이다. VFP는 **개별 인물** 규칙이다
...
| house_rules + episode_style_selection | `house_rules`는 **전역** 문체 규칙, `style_selection`은 **회차별 선택 패턴**. VFP는 **개별** 인물 규칙. 충돌 시 두 문서가 VFP보다 우선 |
...
```
1. house_rules (최상위 — 전역 문체)
2. episode_style_selection (회차별 선택 패턴)
3. voice_fingerprint (개별 인물 — 기질적 말투)
4. Layer A/B culture (게시판/커뮤니티 — 상황적 변조)
5. Big5 + population_grammar (기저 성격 — VFP 없을 때 폴백)
```
