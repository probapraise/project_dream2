...
- latest carry:
  ...
  - `ep004`에서 키리온의 게시판 성과가 공식 인정되며 직접 질문은 줄었지만, 의심이 사라진 것이 아니라 더 조용한 장기 판독으로 옮겨 갔다
...
- latest carry:
  ...
  - `ep003` 코다에서 그 안내판은 날짜/우선순위/직접 수정까지 가능한 참여형 종이 게시판으로 자랐고, 끝내 `아닌데?`를 붙일 자리가 없다는 새 병목을 드러냄
  - `ep004`에서 키리온은 그 결핍을 `댓글`로 명명하고 리리아와 종이 시뮬레이션을 해 보지만, 물리적 한계 앞에서 `수정 1회 + 실명 정정`으로 임시 봉합한 채 마법 탐색으로 다음 수를 넘겼다
...
- latest carry:
  ...
  - `ep004`에서 칼리온의 인정 이후 서고 탐색 폭은 넓어졌지만, 무엇을 계속 다시 펼치는가는 여전히 조용한 판독 자료로 남아 있다
...
- latest carry:
  ...
  - `ep003` 코다에서 집사실 앞 종이 게시판이 실제 참여형 판으로 굴러가다 `댓글/의견/정정 반박` 슬롯의 부재를 드러내며, 장기 커뮤니티 욕망의 다음 결핍이 선명해짐
  - `ep004`에서 `댓글` 욕망은 명시적인 장기 목표가 되었고, 종이 한계 때문에 이를 마법으로 구현해야 한다는 문제의식과 첫 실질 보상까지 함께 지급됐다
...
- latest carry:
  ...
  - 화말 코다에서 그 루틴은 며칠 단위로 실제 반복되기 시작했고, 드문 부재일은 키리온과 리리아가 집사실 앞 종이 게시판을 굴리는 기묘한 틈으로 기능하기 시작했다.
  - `ep004`에서 그 루틴은 2년 가까운 생활 리듬으로 굳었고, 계승조회식 직전 데리온은 더 말이 적고 단단한 상태로 마지막 압박을 건다.
- payoff window: immediate
- linked_arcs:
  - `ARC-001`
