...
작성일: 2026-03-04
최종 업데이트: 2026-03-06 (경로 기준/참조 정합성 동기화)
대상 루트: `/home/dlwhdgus/project_dream2`
조사 방식: 멀티 에이전트 6명 병렬 전수 조사 + 로컬 재검증

## Bootstrap Note

- 이 문서는 사람이 현재 저장소 구조를 빠르게 파악하기 위한 아키텍처 설명서다.
- 오케스트레이션 또는 실행 시작 시 가장 먼저 읽어야 하는 문서는 `world/live/docs/master_map.md` 다.
- 이 문서의 경로 표기는 저장소 루트 기준 실제 경로를 우선 사용한다.
- 단, `world/live/docs/master_map.md` 내부에 적힌 `docs/...`, `world_bible/...`, `population/...` 등은 별도의 bundle-relative 별칭이다.

## 1. 현재 구현 스냅샷

- 학생 모집단 SSOT: `world/live/population/P-*.yaml` + `world/live/population/population_slots.csv`
- 현재 슬롯 총량: `3600`
...
```text
project_dream2/
├─ docs/                             # 루트 설계/아키텍처/핸드오프/히스토리 문서
├─ artifacts/                        # 배치/실행/집필 산출물 보관 구역
│  ├─ batch/                         # 배치 파생지표 산출물
│  ├─ runs/                          # execution gate 산출물
│  └─ writing/                       # 집필 파이프라인 산출물
├─ scripts/                          # 기능군별 실행 스크립트 + legacy entrypoint symlink
│  ├─ population/                    # 모집단 생성/보정/감사
│  ├─ indexes/                       # 인덱스 재생성
│  ├─ ops/                           # world_ops 게이트/케이스 자동화
│  └─ sim/                           # API 시뮬레이션 실행기
├─ world_ops/                        # 변경관리 운영 체계(4-Phase, templates, cases)
├─ world/                            # 안정 경로
│  ├─ live/                          # canonical live bundle
│  │  ├─ docs/                       # 인덱스/상태/워크플로우 문서
│  │  ├─ world_bible/                # 활성 WB 섹션 (PUBLIC/CONFIDENTIAL/META)
│  │  ├─ population/                 # P-* 학생 SSOT + core_cast/
│  │  ├─ nonstudent/                 # NS-* 비학생 슬롯
│  │  ├─ external/                   # EX-* 외곽 인물 슬롯
│  │  ├─ board_states/               # 시뮬레이션 결과 상태
│  │  ├─ layer_b/                    # 활성 게시판 문화 산출물
│  │  ├─ writing/ -> ../../artifacts/writing
│  │  ├─ runs/ -> ../../artifacts/runs
│  │  ├─ quarantine/ -> ../archive/quarantine
│  │  ├─ characters/ -> ../archive/deprecated/characters
│  │  ├─ voice_packs/ -> ../archive/deprecated/voice_packs
│  │  └─ persona_states/ -> ../archive/deprecated/persona_states
│  └─ archive/
│     ├─ deprecated/                 # retired stub systems
│     └─ quarantine/                 # 충돌 가능 레거시 보존
└─ worldbible_refined_bundle_20260303 -> world/live  # legacy compatibility symlink
```
...
- 단일 루트 엔트리: `world/live/docs/master_map.md`
- 오케스트레이터는 `master_map`만 상시 로딩, 나머지는 필요 시 로딩
- 인덱스 포인터:
  - `world/live/docs/world_bible_index_v2.md` (주)
  - `world/live/docs/world_bible_index.md` (legacy)
  - `world/live/docs/character_index_v2.md`
  - `world/live/docs/community_map.md`
  - `world/live/docs/simulation_playbook.md` 등

### 3.2 데이터 레이어 (Population)

- 규범(모델 정의): `world/live/docs/population_grammar.md`
- 실데이터 SSOT:
  - `world/live/population/P-*.yaml` (개별 슬롯 원본)
  - `world/live/population/population_slots.csv` (배치/집계 기준)
- 핵심 필드: `background_type`, `crest_vision_holder`, `dorm`, `grade`, `mana_color`, `major`, `vocation`, Big5, derived

### 3.3 캐릭터 인덱스/명명 레이어

- 슬롯 상태 레지스트리: `world/live/docs/character_index_v2.md`
- 코어캐스트 카드: `world/live/population/core_cast/*.md`
- 코어캐스트 운영 요약: `world/live/docs/core_cast_bootstrap_v1.md`
...
- Layer A: 창발형 **— 문서 완료, 시뮬레이션 3회 실행**
  - `world/live/docs/community_grammar_layer_a.md` (CULTURE 레코드 포맷, 운영 파이프라인, 순응 스펙트럼, 파괴자 감지)
  - 시뮬 결과:
...
- 본문: `world/live/world_bible/WB-*.md`
- 인덱스: `world/live/docs/world_bible_index_v2.md`
- 인덱스 재생성: `scripts/indexes/rebuild_world_indexes.sh`
- 감사: `scripts/ops/world_ops_audit_bundle.sh`

### 5.2 Quarantine 전략

- 목적: 충돌 가능 레거시를 삭제 대신 격리 보존
- 경로: `world/archive/quarantine/` (`world/live/quarantine/` symlink로도 접근 가능)
- 원칙:
  - 활성 문서와 분리
  - 필요 시 선별 재흡수
  - 실행 기본 경로에서는 비활성
...
- 활성 영역 감사는 `world/live/docs`, `world/live/world_bible`, `docs/handoff/next_steps.md` 중심
...
1. **CULTURE 레코드 등재** (즉시)
   - simrun-003 창발 후보 3개 작가 검토 후 `world/live/docs/community_grammar_layer_a.md`에 등재
...
- 생성/초기화:
  ...
  - `scripts/population/generate_nonstudent_slots.py`
...
- 재계산/보정:
  - `scripts/population/recompute_dorms.py`
  - `scripts/population/recompute_role_majors.py`
  - `scripts/population/recompute_derived.py`
  - `scripts/population/apply_population_bootstrap_policy.py`
  - `scripts/population/apply_population_derived_results.py`
- 인덱스/리빌드:
...
- world_ops 운영 게이트:
  - `scripts/ops/world_ops_new_case.sh`
