...
최종 업데이트: 2026-03-15 (world bible 바닥 재정렬 + LMM/그리모어 재설계 케이스 진행 중)
진행 상태 기준: `ep002` canon 확정, world bible 기초 SSOT(`WB-0005/0006/0007/0016/0017/0026/0028/0029`) 정렬 완료, Layer B `ATOM-012` 및 `GRAMMAR-001~003` 초안 반영, `Quick Sim` 1차 파일럿/비교 완료 상태, `CR-20260315-002` 설계 문서화 진행 중
...
- 서사 배치 기준:
  ...
  - 이후 `3년 후`로 점프해 `상급 과정 진입성취평가 -> 입학 즉시 전공 선택 우선권 행사 -> 보존각인학파 선택 클리프행어` 구조로 본편을 연다.
...
### A-0. `CR-20260315-002` LMM/그리모어 재설계 이어가기

이유:
- 오늘 세션에서 `색인의 그리모어 -> 주석의 그리모어 -> 실타래의 그리모어`까지의 전반부 성장선과,
  `요정 동행 -> 드리아드 감리 허가 -> 단계형 공용화 -> 성목 본체 공명`으로 이어지는 후반부 성장선의 큰 틀을 잡았다.
- 다만 아직 `live SSOT`는 수정하지 않았고, 설계 케이스 문서만 쌓인 상태다.

현재 기준점:
- 케이스 폴더: `world_ops/cases/CR-20260315-002/`
- 바로 읽을 문서:
...
- `3년 후` 점프 직전 키리온이 파악하는 각인광장 핵심 병목(배지망, 보존각인 실무, 중계주, 열람권)을 장면 단위로 정리
- 상급 과정 진입성취평가 상위권-전공 선택-보존각인학파 선택 클리프행어의 장면 설계
